...
class RedisBackend {
    ...
    async max() {
        console.info('Connecting to Redis...\r');
        console.time('redis-connect');

        const client = this.connect();

        if (client) {
            console.info('Successfully connected to Redis\r');
        } else {
            throw new Error('Connection to Redis Failed\r');
        }
        ...
        console.info('Disonnecting from Redis...\r');
