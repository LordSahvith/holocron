...
/******/ 	return __webpack_require__(__webpack_require__.s = "./cartridges/plugin_cricut_pagedesigner/cartridge/client/default/js/assets/hero.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./cartridges/app_cricut/cartridge/client/default/js/components/contentModal.js":
/*!**************************************************************************************!*\
  !*** ./cartridges/app_cricut/cartridge/client/default/js/components/contentModal.js ***!
  \**************************************************************************************/
...
/**
 * Open Video Modal and insert video url
 * @param {HTML} content the content that goes in the modal
 * @param {string} modalClass modal class
 * @returns {HTML} modal
 */
function buildModalHtml(content, modalClass) {
    var $modal =
    `<div class="modal modal--content ${modalClass}" id="content-modal" tabindex="-1" role="dialog" aria-labelledby="modal" aria-hidden="true">
...
/**
 * close modal if user pressed escape key
 * @param {Object} e key event
 */
function checkForEscapeKey(e) {
    // get key code - cross browser supported
...
function setModalEvent() {
    var trigger = $('[data-toggle="content-modal"]');
    ...
    trigger.on('click', function(e) {
        ...
        // Expect a Video Tag modal
        if ($(this).data('video-url')) {
            contentSRC = $(this).data('video-url');
            modalClass = 'video-modal';

            contentHtml =
            `<div class="video-container">
                <video src="${contentSRC}" controls autoplay playsinline></video>
            </div>`;

        }

        // Expect an embedded YouTube Video modal
        if ($(this).data('youtube-url')) {
            contentSRC = $(this).data('youtube-url');
            modalClass = 'video-modal';
        ...
        // Expect an external page iframe modal
        if ($(this).data('content-url')) {
            contentSRC = $(this).data('content-url');
            modalClass = 'content-modal';
            let title = $(this).data('title');

            contentHtml =
            `<div class="content-container">
                <iframe type='text/html' src="${contentSRC}" title="${title}" frameBorder="0" allow="accelerometer" encrypted-media allowfullscreen></iframe>
            </div>`;
        }
        
        // expect a static or HTML modal
        if ($(this).data('static-content')) {
            // TODO:: static content version
        }

        // remove any content modal that was left on the page
        if ($('.modal--content.' + modalClass).length) {
            $('.modal--content.' + modalClass).remove();
        }

        // make our new modal HTML
        var $modal = buildModalHtml(contentHtml, modalClass);
        $('body #maincontent').append($modal);

        // if it's a YouTube modal we need to initialize it on open now that it's in the DOM
        if ($(this).data('youtube-url')) {
            $('.modal--content.' + modalClass).on('shown.bs.modal', function () {
        ...
        $('.modal--content.' + modalClass).modal('show');

        $('.modal--content button.close').on('click', function () {
            closeModal();
        });

        window.addEventListener('keyup', checkForEscapeKey);
    });
}
...
/***/ "./cartridges/plugin_cricut_pagedesigner/cartridge/client/default/js/assets/hero.js":
/*!******************************************************************************************!*\
  !*** ./cartridges/plugin_cricut_pagedesigner/cartridge/client/default/js/assets/hero.js ***!
  \******************************************************************************************/
/*! no exports provided */
...
var contentModal = __webpack_require__(/*! base/components/contentModal */ "./cartridges/app_cricut/cartridge/client/default/js/components/contentModal.js");
...
function Hero(hero) {
    ...
    this.createHeroWide = () => {
        ...
        this.scrollAnim = gsap__WEBPACK_IMPORTED_MODULE_0__["gsap"].timeline({
            scrollTrigger: {
                start: 0,
                end: y,
                scrub: 0.2,
        ...
        const loadAnim = gsap__WEBPACK_IMPORTED_MODULE_0__["gsap"].timeline({
            paused: true,
            onComplete: () => {
                //this.enableScroll();
    ...
    this.createHeroSplit = () => {
        ...
        const gap = 24;
        gsap__WEBPACK_IMPORTED_MODULE_0__["gsap"].set(this.product, {
            xPercent: -50,
            x: -gap,
        });
        gsap__WEBPACK_IMPORTED_MODULE_0__["gsap"].set(this.title, {
            xPercent: 50,
            x: gap,
            opacity: 0
        });
    ...
    gsap_CustomEase__WEBPACK_IMPORTED_MODULE_2__["CustomEase"].create("heroEase", "0.48, 0.04, 0.52, 0.96");
